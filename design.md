...
- Don't reach into the shadcn-compat tokens (`legacy.light.json` / `legacy.dark.json`) in new code. They expose bare names like `--background`, `--foreground`, `--card`, `--popover`, `--muted`, `--accent`, `--destructive`, `--ring`, `--input-background`, `--switch-background` so Tailwind v4's `@theme` block (and any shadcn-style class such as `bg-background`, `text-foreground`) can resolve. They're load-bearing — a few internal components (`timeline.tsx`, `sonner.tsx`) and Tailwind itself currently depend on them — but the file is named `legacy.*` because new code should reach for the underlying semantic tokens (`bg.base`, `text.primary`, `bg.muted`, `danger.600`, `interactive.default`, etc.) instead. Migrating those last consumers and dropping the alias file is tracked for a future major version.
